...
function triggerConnection(state, extra) {
    fakeSocket._ev.emit('connection.update', { connection: state, ...extra });
}
function triggerDisconnect(statusCode) {
...
async function triggerMessages(messages) {
...
describe('WhatsAppChannel', () => {
    ...
    describe('connection lifecycle', () => {
        ...
        it('flushes outgoing queue on reconnect', async () => {
            ...
            await channel.flushOutgoingQueue();
            // Group messages get prefixed when flushed
            expect(fakeSocket.sendMessage).toHaveBeenCalledWith('test@g.us', { text: 'Andy: Queued message' });
        });
    ...
    describe('message handling', () => {
        it('delivers message for registered group', async () => {
            ...
            expect(opts.onMessage).toHaveBeenCalledWith('registered@g.us', expect.objectContaining({
                id: 'msg-1',
                content: 'Hello Andy',
                sender_name: 'Alice',
                is_from_me: false,
