...
│  │    • mcp__zenthor__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── zenthor.log               # Host stdout
│   └── zenthor.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.zenthor.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || "zenthor-agent:latest";
...
- `{{PROJECT_ROOT}}` - Absolute path to your zenthor installation
...
Zenthor uses a hierarchical memory system based on CLAUDE.md files.
...
Zenthor has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__zenthor__schedule_task]
        {
...
### Zenthor MCP (built-in)

The `zenthor` MCP server is created dynamically per agent call with the current group's context.
...
Zenthor runs as a single macOS launchd service.

### Startup Sequence

When Zenthor starts, it:

1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned Zenthor containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/zenthor.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/zenthor.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.zenthor.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.zenthor.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.zenthor.plist

# Check status
launchctl list | grep zenthor

# View logs
tail -f logs/zenthor.log
```
...
| ---------------------------------------- | --------------------------------- | ---------------------------------------------------------------------------------------- | -------------- |
| No response to messages                  | Service not running               | Check `launchctl list                                                                    | grep zenthor` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; Zenthor auto-starts container runtime but may fail                          |
...
- `logs/zenthor.log` - stdout
- `logs/zenthor.error.log` - stderr
